...
exports.findOne = async (req, res, next) => {
    try {
        ...
        if (!ingredient) return res.status(404).json({ error: 'Ingredients not found' }); // 404 Not Found
...
exports.update = async (req, res, next) => {
    try {
        ...
        if (!updated) return res.status(404).json({ error: 'Ingredients not found' }); // 404 Not Found
...
exports.delete = async (req, res, next) => {
    try {
        ...
        if (deleted === 0) return res.status(404).json({ error: 'Ingredients not found' });
